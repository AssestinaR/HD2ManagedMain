...
public sealed class DecorationPlanPageViewModel : PageViewModel
{
    ...
    private async Task LoadAsync()
    {
        try
        {
            ...
            if (result.Data is not null)
            {
                ...
                _allSourceUnits = catalogEntries
                    .SelectMany(entry => entry.Parts)
                    .GroupBy(part => new { part.UnitAssetKey, part.PartKind, part.BodyVariant, part.Layer })
    ...
    private async Task GenerateAsync()
    {
        var plan = new DecorationPlanDocument
        {
            ...
            TargetBodyVariant = TargetBodyVariant,
            DualVariantMode = DualVariantMode,
            TargetModIds = TargetMods.Where(item => item.IsSelected).Select(item => item.ModId).ToList(),
            SourceUnits = SourceUnits.Where(item => item.IsSelected).Select(item => new DecorationSourceUnit { TypeId = item.TypeId, FileId = item.FileId, IncludeCulling = item.IncludeCulling }).ToList()
        };
        var displayName = $"{SourceName} - 装饰";
        if (AutoImport)
        {
            var created = await _library.CreateDecorationAsync(SourceModId, plan, displayName);
        ...
        var path = Path.Combine(OutputDirectory, $"{SanitizeFileName(displayName)}-plan.json");
        await File.WriteAllTextAsync(path, JsonSerializer.Serialize(plan, new JsonSerializerOptions(JsonSerializerDefaults.Web) { WriteIndented = true }));
        State = $"计划已写出：{path}";
