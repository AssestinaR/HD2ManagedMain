...
public sealed class ConflictDetectorTests
{
	...
	public async Task DetectNodeConflictsAsync_FindsIntersection()
	{
		...
		try
		{
			...
			var nodeA = new ModNode(idA, "a", new ModNodeMetadata("a", null, Array.Empty<string>(), null, DateTimeOffset.UtcNow, null), Array.Empty<PatchGroupKey>(), Array.Empty<ModNodeId>());
			var nodeB = new ModNode(idB, "b", new ModNodeMetadata("b", null, Array.Empty<string>(), null, DateTimeOffset.UtcNow, null), Array.Empty<PatchGroupKey>(), Array.Empty<ModNodeId>());
