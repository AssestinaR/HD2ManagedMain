...
// Purpose: Selects source RawMesh replacements for target Unit slots using mesh semantics and stream structure.
public sealed class UnitMeshReplacementStrategy
{
	...
	public static IReadOnlyList<UnitMeshReplacementCandidate> SelectNonConflictingCandidates(IReadOnlyList<UnitMeshReplacementCandidate> candidates)
	{
	...
	private static bool HasSameStreamLayout(UnitStreamInfo targetStream, UnitStreamInfo sourceStream)
	{
		...
		for (var index = 0; index < targetStream.Components.Count; index++)
		{
			var target = targetStream.Components[index];
			var source = sourceStream.Components[index];
	...
	private static bool HasSameMaterialSlots(UnitRawMeshData targetRawMesh, UnitRawMeshData sourceRawMesh)
	{
		...
		for (var index = 0; index < targetRawMesh.Sections.Count; index++)
		{
			if (targetRawMesh.Sections[index].MaterialSlotId != sourceRawMesh.Sections[index].MaterialSlotId)
			{
				return false;
			}
		}
	...
	private static int CalculateScore(UnitMeshReplacementCandidateKind kind, UnitRawMeshData targetRawMesh, UnitRawMeshData sourceRawMesh, UnitStreamInfo targetStream, UnitStreamInfo sourceStream, bool sameStreamLayout, bool sameVertexStride, int semanticMatch)
	{
	...
	private static IReadOnlyList<UnitMeshReplacementComponentSignature> BuildComponentLayout(UnitStreamInfo stream)
		=> stream.Components.Select(component => new UnitMeshReplacementComponentSignature(component.Type, component.Format, component.Index, component.Size)).ToArray();
