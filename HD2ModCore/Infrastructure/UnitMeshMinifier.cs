...
// 作用：实现 Unit RawMesh 极小化，把 mesh 替换为保留原顶点格式的占位三角形。
// Purpose: Minifies Unit RawMeshes by replacing mesh geometry with placeholder triangles that preserve the stream vertex format.
public sealed class UnitMeshMinifier : IUnitMeshMinifier
{
	...
	public UnitMeshModel MinifyRawMesh(UnitMeshModel model, int meshInfoIndex)
	{
		return Minify(model, new HashSet<int> { meshInfoIndex });
	}
	...
	{
		return (uint)Math.Clamp((int)MathF.Round(value * max), 0, (int)max);
	}
